...
BBNode::BBNode(IloIntArray *sj, IloIntArray *pj, IloIntArray* dj,
		vector<int> jobs_in_batch, vector<int> jobs_in_rest,
		int* Lmax_incumbent, vector<int>* best_solution,
		vector<int>* current_solution, int starttime, int k, int* nk,
		int capacity, int Dmax, int* nodesVisited, double* timeCounter) {

	//cout << "Creating child" << endl;
	//copy all parameters into variables for later use
...
BBNode::~BBNode() {
	// Cplex garbage collection ...
	// right now, there's nothing here because it's all RootNode's business,
	// but it's important to do this or else the tree fills up the RAM immediately.
	// sadly, this part is pretty slow.
...
#ifdef BENDER_MIP // I've put both MIP and CP versions in the same file; they're selected via compiler argument
int BBNode::run() {
	...
	// restriction on any job's finish time based on its lft and what's in the batch
	...
	while(cplex.solve()) { // keep solving until there are no more children
		...
		child->run(); // let the child take care of things, wait until it's done (= until it backtracks).
