...
int RootNode::run() {
	...
	is_inbatch = IloNumVarArray(env, all_jobs->size());
	for(int j=0; j<all_jobs->size(); j++) {
	...
	while(cplex.solve()) { // keep solving until there are no more children
		...
		for(int i=0; i<all_jobs->size(); i++) {
			if(cplex.getValue(is_inbatch[i]) > 0) currentSolutionSum += is_inbatch[child_jobs_in_batch[i]];
		}
		cout << "This was root node ";
		for(int j=0; j<all_jobs->size(); j++) {
