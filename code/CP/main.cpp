...
IloIntExpr secondarySumExpr(IloEnv env, IloIntArray coeff,
...
/*** Custom propagators ***/
IloConstraint MinNonzeroDuedate(IloIntVar Dk, IloIntVarArray assignments,
		IloIntArray dj, int k) {
	return IloCustomConstraint(Dk.getEnv(),
			new (Dk.getEnv()) MinNonzeroDuedateI(Dk, assignments, dj, k));
}

IlcConstraint IlcMaxProcessingTime(IloCP cp, IlcIntervalVar K,
		IlcIntVarArray assignments, IlcIntArray pj, int k) {
	//IloCP cp = K.getCP();
...
int main(int argc, const char * argv[]) {
	...
	try {
		...
		for (int j = 0; j < nj; j++) {
			assignments[j] = IloIntVar(env, 0, nk,
					charcat("assignment_j", j, 0));
		}

		// instantiate Dk and Pk
		for (int k = 0; k < nk; k++) {
			Dk[k] = IloIntVar(env, 0, Dmax, charcat("Dk_", k, 0));
			batchloads[k] = IloIntVar(env, 0, capacity,
					charcat("batchload_k", k, 0));
		...
		for (int k = 0; k < nk; k++) {
			IloIntExprArray prodExpression = secondaryProdArray(env, pj, xjk, k,
					nj);
		...
		for (int j = 0; j < nj; j++) {
			...
			if (Cmax_LB_temp - dj[j] > Lmax_LB) {
				...
				cout << "Updating LmaxLB to " << Lmax_LB << ". C="
						<< Cmax_LB_temp << ", d=" << dj[j];
		...
		for (int k = 0; k < nk - 2; k++) {
			model.add(
					IloIfThen(env, IloEndOf(K[k + 2]) > IloEndOf(K[k]),
