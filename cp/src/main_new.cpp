...
int main(int argc, const char * argv[]){
  ...
  //      model.add( xjk[j][k] == (assignments[j]==k) );
    ...
    for(int k=0; k<nk; k++) {
      ...
      IloIntExprArray P_expression(env);
      IloIntExprArray D_expression(env);
      for(int j=0; j<nj; j++) {
        P_expression.add( pj[j] * (assignments[j]==k) );
        D_expression.add( dj[j] + Dmax * (assignments[j]>k) + Dmax *
        (assignments[j]<k));
        //model.add( IloLengthOf(J[j]) == IloLengthOf(K[k]) * xjk[j][k] +
        //IloLengthOf(J[j])*(1-xjk[j][k]));
      }
        model.add( IloLengthOf(K[k]) == IloMax(P_expression) );
        model.add( Dk[k] == IloMin(D_expression));
    }
   ...
   //   model.add(IloIfThen( env, IloEndOf(K[k+2]) > IloEndOf(K[k]),
    ...
    for(int j=0; j<nj; j++) {
      ...
      cout << (j);
      for(int k=0; k<nk; k++) {
        cout << (cp.getValue(assignments[j]==k) == true ? " X" : " ·");
      }
      cout << endl;
    }
