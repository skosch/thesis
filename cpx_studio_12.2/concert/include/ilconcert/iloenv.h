...
public:
	...
	virtual void shareCollection(IloDataCollectionI* coll) = 0;
	virtual void endCollection(IloDataCollectionI*) = 0;
	virtual IloBool isSharing(IloDataCollectionI*) const = 0;
...
inline IloNum IloLog10(float x) { return log10(x); }/* using log10() from <math.h> */
inline IloNum IloLog2(float x) { return IloLog(x)/IloLog(2); }/* using log() from <math.h> */
...
public:
	...
	IloArrayI* copy() const {
		...
		IloMemoryManagerI* env = _impl->getMemoryManager();
...
//------------------------------------------------------------
class IloObservableI;
class IloObserverI {
	IloObservableI* _obs;
public:
	IloObserverI(): _obs(0){};
	virtual ~IloObserverI();
	virtual void update( const void* observable, void* eventType ) = 0;
	void setObservable(IloObservableI* obj){ _obs = obj; };
};

class IloObservableI {
public:
	IloObservableI( IloEnvI* env ) ;
	virtual ~IloObservableI();

	void addObserver( IloObserverI* obj );
	void removeObserver( IloObserverI* obj );

	void notifyObservers( const void* observable, void* eventType ) const;
	void notifyObservers( void* eventType = 0 ) const;
private:
	IloEnvI* _env;
	IloArray< IloObserverI* > _observerList;
};
...
#if defined(ILO_WINDOWS) && defined(ILO_MSVC8) && !defined(_M_IA64)
template< int X >
