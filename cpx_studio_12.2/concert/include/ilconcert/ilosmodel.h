...
class IloIsomorphic: public IloConstraint {
  ILOEXTRHANDLE(IloIsomorphic, IloConstraint)
    public:

  IloIsomorphic(const IloEnv env, 
                const IloIntervalVarArray domain,
                const IloIntervalVarArray codomain,
                const char* name =0) {
    IloAssert(env.getImpl(), "IloEnv: empty handle.");
    IloAssert(domain.getImpl(), "IloIntervalVarArray: empty handle.");
    IloAssert(codomain.getImpl(), "IloIntervalVarArray: empty handle.");
    IloAssert(domain.getSize()==codomain.getSize(), "IloIsomorphic: domain and codomain arrays have different size.");
#ifndef NDEBUG
    for (IloInt i = 0; i < domain.getSize(); i++) {
      IloAssert(domain[i].getImpl(), "IloIntervaVarArray: contains empty handle.");
      IloAssert(codomain[i].getImpl(), "IloIntervaVarArray: contains empty handle.");
    }
#endif
    _ctor(env, domain, codomain, name);
  }
  ...
  IloIsomorphic(const IloEnv env, 
    ...
    IloAssert(domain.getSize()==codomain.getSize(), "IloIsomorphic: domain and codomain arrays have different size.");
    IloAssert(domain.getSize()==map.getSize(), "IloIsomorphic: domain and map arrays have different size.");
#ifndef NDEBUG
    for (IloInt i = 0; i < domain.getSize(); i++) {
      IloAssert(domain[i].getImpl(), "IloIntervaVarArray: contains empty handle.");
      IloAssert(codomain[i].getImpl(), "IloIntervaVarArray: contains empty handle.");
      IloAssert(map[i].getImpl(), "IloIntExprArray: contains empty handle.");
    }
#endif
    _ctor(env, domain, codomain, map, name);
  }
private:
  void _ctor(const IloEnv env, 
             const IloIntervalVarArray domain,
...
typedef IloArray<IloIsomorphic> IloIsomorphicArray;
