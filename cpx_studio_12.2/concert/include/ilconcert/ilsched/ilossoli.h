...
public:
  ...
  public:
    ...
    NodeI* init(IloExtractableI* extr);
    virtual ~NodeI(){}
...
private:
  NodeI* newNode(IloExtractableI* extr);
  ArcI*  newArc(NodeI* source, NodeI* target, IloUInt type);
  void removeArc(ArcI* arc);
  void recycle(NodeI* node){ _recycledNodes.add(_m, node); }
...
public:
  ...
  NodeI* makeNode(IloExtractableI* e);
