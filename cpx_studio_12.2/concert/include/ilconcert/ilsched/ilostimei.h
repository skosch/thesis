...
class IloIsomorphicI: public IloConstraintI {
  ILOS_CPEXTR_DECL(IloIsomorphicI, IloConstraintI)
private:
  IloExtractableArray _domain;
  IloExtractableArray _codomain;
  IloIntExprArray     _map;
  IloIsomorphicI(IloEnvI* env, const char* name);

public:
  IloIsomorphicI(IloEnvI* env, IloExtractableArray domain, IloExtractableArray codomain, const char* name);
  IloIsomorphicI(IloEnvI* env, IloExtractableArray domain, IloExtractableArray codomain, IloIntExprArray map, const char* name);
  virtual ~IloIsomorphicI();
  IloExtractableArray getDomain()   const { return _domain; }
  IloExtractableArray getCodomain() const { return _codomain; }
