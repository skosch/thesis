...
IloCumulFunctionExprArray DefineWorkers(IloEnv env) {
  /*
    Workers are cumul functions with some inititail usage
   */
...
ILCGOAL1(RelaxGreedySearchGoal,
  ...
  */
  ...
  IloInt guilty;
  IlcGoal g = IlcAnd(IlcGoal(jobCt),
		     GreedyListSchedulingGoal(cp, s, guilty));
  IloInt iter = 0;
  while(IloTrue) {
...
RelaxGreedySearch::RelaxGreedySearch(IloCP cp,
				     IloAnyArray jobs)
...
IloBool
RelaxGreedySearch::select(IloInt& job,
			  IloInt& interval,
			  IloInt& date) const {
  /*
