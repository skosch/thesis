...
public:
  ...
  IlcIntExp getMakespan() const { return _makespan;}
...
SetTimesSearch::SetTimesSearch(IloCP cp,
  ...
  ,_postponedDates(0)
  ,_makespan(cp.getIntExp(makespan))
  ,_policy(policy)
{
...
void SetTimesSearch::schedule(IlcIntervalVar var,
			      IloInt date) {
  if (var.isAbsent()) 
    return;
...
void SetTimesSearch::postpone(IlcInt index,
			      IloInt date) {
  IloCP cp = getCP();
...
IloBool SetTimesSearch::tryPresent(IlcIntervalVar var,
				   IloInt date) {
  IloCP cp = getCP();
  if (var.isAbsent())
    return IloFalse;
    ...
    IloInt smin;
    if (!cp.solve(TryPresentGoal(cp, var, smin),
		  IloTrue)) {
      var.setAbsent();
      return IloFalse;
    }
...
public:
  ...
  IloIntExprArg getMakespan() const { return _makespan;}
...
int main(int argc, const char* argv[]){
  ...
  try {
    ...
    IloIntExprArg makespan = IloMax(ends);
	...
	new (env) RCPSPMMScheduler(intervals, makespan, policy);
      switch(policy) {
