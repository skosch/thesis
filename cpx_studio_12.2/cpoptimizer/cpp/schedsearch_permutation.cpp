...
private:
  ...
  void extendHead(IlcIntervalSequenceVar s1,
		  IlcIntervalVar ti1);
  void syncHead();
  void moveTailEarlier();
  void extendTail(IlcIntervalSequenceVar s1,
		  IlcIntervalVar ti1);  
  void syncTail();
  void checkHeadNeighbour(IlcIntervalSequenceVar s,
			  IloInt index);
  void checkTailNeighbour(IlcIntervalSequenceVar s,
			  IloInt index);
  void propagateHead(IlcIntervalSequenceVar sequence,
		     IlcIntervalVar from);
  void propagateTail(IlcIntervalSequenceVar sequence,
		     IlcIntervalVar from);
  void propagatePresence(IlcIntervalSequenceVar sequence,
			 IlcIntervalVar var);
...
public:
  ...
  IlcIntervalVar getInterval(IloInt index,
			     IlcIntervalSequenceVar s) const {
    return (index < 0) ? 0
      : ((s.getImpl() == _s1.getImpl())
	 ? _intervals1[index] : _intervals2[index]);
  }
  ...
  void propagateDeltaHead1() {
    ...
    propagateHead(sequence, 
		  sequence.getEarliestNewInHead());
  }
  ...
  void propagateDeltaTail1()  {
    IlcIntervalSequenceVar sequence = getSequence1();
    propagateTail(sequence, 
		  sequence.getEarliestNewInTail());
  }
  ...
  void propagateDeltaPresence1() {
    IlcIntervalSequenceVar sequence = getSequence1();
    propagatePresence(sequence,
		      sequence.getDeltaPresence());
  }
...
ILOCPCONSTRAINTWRAPPER5(IloSamePermutation, cp,
  ...
  return new (cp.getHeap())
    SamePermutationCtI(cp, cs1, cs2,
		       intervals1, intervals2, same);
}
...
void MakeConveyorBelt(IloModel model,
  ...
  for(IloInt i = 0; i < nb; ++nb) {
    ...
    if (!optionals[i]) {
      ...
      model.add(IloPresenceOf(env, load) ==
		IloPresenceOf(env, trans));
      model.add(IloPresenceOf(env, load) ==
		IloPresenceOf(env, unload));
    }
