           ...
           int        *useraction_p)
{
   ...
   *useraction_p = CPX_CALLBACK_DEFAULT;
   ...
   if ( nodecount < 1 ) {
      ...
      status = CPXXgetcntparam (env, CPX_PARAM_ITLIM, &itlimit);
      if ( status )  goto TERMINATE;

      status = CPXXsetcntparam ((CPXENVptr) env, CPX_PARAM_ITLIM, 200);
      if ( status )  goto TERMINATE;

      status = CPXXdualopt (env, nodelp);

      CPXXsetcntparam ((CPXENVptr) env, CPX_PARAM_ITLIM, itlimit);
      ...
      if ( !status ) {
         ...
         if ( solstat == CPX_STAT_ABORT_IT_LIM ) {
            ...
            status = CPXXgetintparam (env, CPX_PARAM_PREIND, &preind);
            if ( status )  goto TERMINATE;
            status = CPXXgetintparam (env, CPX_PARAM_AGGIND, &aggind);
            if ( status )  goto TERMINATE;

            status = CPXXsetintparam ((CPXENVptr) env, CPX_PARAM_PREIND, CPX_ON);
            if ( status )  goto TERMINATE;
            status = CPXXsetintparam ((CPXENVptr) env, CPX_PARAM_AGGIND, -1);
            if ( status )  goto TERMINATE;

            status = CPXXhybbaropt (env, nodelp, CPX_ALG_AUTOMATIC);

            CPXXsetintparam ((CPXENVptr) env, CPX_PARAM_PREIND, preind);
            CPXXsetintparam ((CPXENVptr) env, CPX_PARAM_AGGIND, aggind);
         }
      }
   }
