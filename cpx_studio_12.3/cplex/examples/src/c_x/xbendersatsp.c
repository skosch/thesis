...
main (int  argc, char *argv[])
{
   ...
   status = CPXXsetlongparam (env, CPX_PARAM_MIPINTERVAL, 1);
   if ( status )  {
      fprintf (stderr,
                     ...
                     int separate_fractional_solutions)
{
   ...
   status = CPXXsetintparam (user_cbhandle->env, CPX_PARAM_REDUCE, 0);
   if ( status ) {
      fprintf(stderr, 
...
set_benders_callback  (CPXENVptr env, USER_CBHANDLE *user_cbhandle)
{
   ...
   status = CPXXsetintparam (env, CPX_PARAM_THREADS, 1);
   if ( status )  {
      fprintf (stderr,
