...
main (void)
{
   ...
   status = CPXXsetintparam (env, CPX_PARAM_SCRIND, CPX_OFF);
   if ( status ) {
      fprintf (stderr, 
   ...
   status = CPXXsetintparam (env, CPX_PARAM_LPMETHOD, CPX_ALG_PRIMAL);
   if ( status ) {
      fprintf (stderr, 
...
populatebycolumn (CPXENVptr env, CPXLPptr lp)
{
   ...
   CPXXchgobjsen (env, lp, CPX_MAX);  /* Problem is maximization */
