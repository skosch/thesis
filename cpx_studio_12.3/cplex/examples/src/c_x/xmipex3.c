...
main (void)
{
   ...
   status = CPXXsetintparam (env, CPX_PARAM_PREIND, CPX_OFF);
   if (!status) CPXXsetintparam (env, CPX_PARAM_AGGIND, CPX_OFF);
   if (!status) CPXXsetcntparam (env, CPX_PARAM_MIPINTERVAL, 1);
