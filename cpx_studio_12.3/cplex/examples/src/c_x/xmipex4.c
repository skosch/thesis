...
main (int argc, char *argv[])
{
   ...
   status = CPXXsetcntparam (env, CPX_PARAM_NODELIM, 5000);
   ...
   if ( usetimelimcallback ) {
      ...
      status = CPXXsetintparam (env, CPX_PARAM_MIPDISPLAY, 0);
      if ( status )  goto TERMINATE;
   }
...
logcallback (CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle)
{
   ...
   if ( nodecnt >= info->lastlog + 100  ||  newincumbent ) {
      ...
      printf ("Nodes = %lld(%lld)  Best objective = %g", nodecnt, nodesleft, bound);
