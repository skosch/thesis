...
class Cplex(object):
        ...
        if len(args) > 0 and isinstance(args[0], Cplex):
            ...
            if len(args) > 0 and isinstance(args[0], type("")):
                ...
                if len(args) > 1 and isinstance(args[1], type("")):
                    filetype = args[1]
                self._lp = _internal._procedural.createprob(env._e, filename)
                _internal._procedural.readcopyprob(env._e, self._lp, filename, filetype)
            else:
                self._lp = _internal._procedural.createprob(env._e, "")
        self._env              = env
    ...
    def write(self, filename, filetype = ""):
        ...
        if filetype == "":
            ...
            filetype = filetype[1:]
        if filetype in ["sav", "mps", "lp", "rew", "rlp"]:
    ...
    def get_problem_name(self):
        ...
        return _internal._procedural.getprobname(self._env._e, self._lp)

    def set_problem_name(self, name):
        """Sets the problem name."""
        _internal._procedural.chgprobname(self._env._e, self._lp, name)
