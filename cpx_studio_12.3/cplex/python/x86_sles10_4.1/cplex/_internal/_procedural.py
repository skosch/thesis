...
def Pylolmat_to_CHBmat(lolmat, get_indices, r_c):
    return CR.Pylolmat_to_CHBmat(lolmat, get_indices, r_c)
...
def getparamname(env, whichparam):
    output = []
...
def newcols(env, lp, obj, lb, ub, xctype, colname):
    ...
    status = CR.CPXXnewcols(env, lp, ccnt, obj.array,
                           lb.array, ub.array, xctype, colname)
    check_status(env, status)
    return
...
def chgcolname(env, lp, indices, newnames):
    status = CR.CPXXchgcolname(env, lp, len(indices), LAU.int_list_to_array(indices), newnames)
    check_status(env, status)
    return
...
def addrows(env, lp, ccnt, rcnt, nzcnt, rhs, sense, rmat, colname, rowname):
    ...
    status = CR.CPXXaddrows(env, lp, ccnt, rcnt, nzcnt, rhs.array, 
                           sense, rmat._mat, colname, rowname)
    check_status(env, status)
    return
...
def chgrowname(env, lp, indices, newnames):
    status = CR.CPXXchgrowname(env, lp, len(indices), LAU.int_list_to_array(indices), newnames)
    check_status(env, status)
    return
...
def addlazyconstraints(env, lp, rhs, sense, rmatbeg, rmatind, rmatval, names):
    status = CR.CPXXaddlazyconstraints(env, lp, len(rmatbeg), len(rmatind),
                                      ...
                                      LAU.double_list_to_array(rmatval), names)
    check_status(env, status)
    return
...
def addsos(env, lp, sostype, sosbeg, sosind, soswt, sosnames):
    status = CR.CPXXaddsos(env, lp, len(sosbeg), len(sosind), sostype,
                          LAU.int_list_to_array(sosbeg), LAU.int_list_to_array(sosind),
                          LAU.double_list_to_array(soswt), sosnames)
    check_status(env, status)
    return
...
def getsosindex(env, lp, name):
    index  = CR.intPtr()
    status = CR.CPXXgetsosindex(env, lp, name, index)
    check_status(env, status)
    return index.value()
...
def addindconstr(env, lp, indvar, complemented, rhs, sense, linind, linval, name):
    status = CR.CPXXaddindconstr(env, lp, indvar, complemented, len(linind), rhs, sense,
                                LAU.int_list_to_array(linind), LAU.double_list_to_array(linval), name)
    check_status(env, status)
    return
...
def getindconstrindex(env, lp, name):
    index = CR.intPtr()
    status = CR.CPXXgetindconstrindex(env, lp, name, index)
    check_status(env, status)
    return index.value()
...
def addqconstr(env, lp, rhs, sense, linind, linval, quadrow, quadcol, quadval, name):
    status = CR.CPXXaddqconstr(env, lp, len(linind), len(quadrow), rhs, sense,
                              ...
                              LAU.double_list_to_array(quadval), name)
    check_status(env, status)
    return
...
def getqconstrindex(env, lp, name):
    index = CR.intPtr()
    status = CR.CPXXgetqconstrindex(env, lp, name, index)
    check_status(env, status)
    return index.value()
...
def chgobjsen(env, lp, maxormin):
    CR.CPXXchgobjsen(env, lp, maxormin)
    return
...
def addmipstarts(env, lp, beg, varindices, values, effortlevel, mipstartname):
    status = CR.CPXXaddmipstarts(env, lp, len(beg), len(varindices), LAU.int_list_to_array(beg),
                                LAU.int_list_to_array(varindices), LAU.double_list_to_array(values),
                                LAU.int_list_to_array(effortlevel), mipstartname)
    check_status(env, status)
    return
...
def branchcallbackbranchgeneral(cbstruct, nodeest,
                                ind, lu, bd, rhs, sense, matbeg, matind, matval, userhandle):
    seqnum = CR.cpxlongPtr()
    status = CR.CPXXbranchcallbackbranchgeneral(cbstruct, nodeest, len(ind),
                                               LAU.int_list_to_array(ind), lu,
                                               LAU.int_list_to_array(bd), len(matbeg), len(matind),
                                               LAU.double_list_to_array(rhs), sense, 
...
def addsolnpooldivfilter(env, lp, lb, ub, ind, wts, ref, name):
    status = CR.CPXXaddsolnpooldivfilter(env, lp, lb, ub, len(ind),
                                        ...
                                        LAU.double_list_to_array(ref), name)
    check_status(env, status)
    return
...
def addsolnpoolrngfilter(env, lp, lb, ub, ind, val, name):
    status = CR.CPXXaddsolnpoolrngfilter(env, lp, lb, ub, len(ind),
                                        LAU.int_list_to_array(ind),
                                        LAU.double_list_to_array(val), name)
    check_status(env, status)
    return
