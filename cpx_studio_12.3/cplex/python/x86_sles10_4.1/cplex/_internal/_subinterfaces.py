...
class IndexedInterface(BaseInterface):
    ...
    def get_indices(self, name):
        ...
        """
        if isinstance(name, type("")):
            return self._get_index_function(self._env._e, self._cplex._lp, name)
        else:
            return map(lambda a:  self._get_index_function(self._env._e, self._cplex._lp, a), name)
...
class VariablesInterface(IndexedInterface):
    ...
    def add(self, obj = [], lb = [], ub = [], types = "", names = [], columns = []):
        ...
        if columns == []:
            CPX_PROC.newcols(self._env._e, self._cplex._lp, obj, lb, ub, types, names)
        else:
            ...
            CPX_PROC.addcols(self._env._e, self._cplex._lp, num_new_cols, cmat._get_nnz(), obj,
                             cmat, lb, ub, names)
            if types != "":
                CPX_PROC.chgctype(self._env._e, self._cplex._lp, range(num_new_cols), types)
    ...
    def get_names(self, *args):
        ...
        def getname(a, b = self.get_num() - 1):
            return CPX_PROC.getcolname(self._env._e, self._cplex._lp, a, b)
...
class LinearConstraintInterface(IndexedInterface):
    ...
    def add(self, lin_expr = [], senses = "", rhs = [], range_values = [], names = []):
        ...
        if lin_expr == []:
            ...
            CPX_PROC.newrows(self._env._e, self._cplex._lp, rhs, senses, range_values, names)
        else:
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a, b = self.get_num() - 1):
            return CPX_PROC.getrowname(self._env._e, self._cplex._lp, a, b)
...
class IndicatorConstraintInterface(IndexedInterface):
    ...
    def add(self, lin_expr = SparsePair(), sense = "E", rhs = 0.0, indvar = 0, complemented = 0, name = ""):
        ...
        CPX_PROC.addindconstr(self._env._e, self._cplex._lp, self._cplex.variables._conv(indvar), complemented,
                              rhs, sense, map(self._cplex.variables._conv, ind), val, name)
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a):
            return CPX_PROC.getindconstrname(self._env._e, self._cplex._lp, a)
...
class QuadraticConstraintInterface(IndexedInterface):
    ...
    def add(self, lin_expr = SparsePair([0], [0.0]), quad_expr = SparseTriple([0], [0], [0.0]), sense = "L", rhs = 0.0, name = ""):
        ...
        CPX_PROC.addqconstr(self._env._e, self._cplex._lp, rhs, sense, map(self._cplex.variables._conv, ind), val,
                            map(self._cplex.variables._conv, ind1), map(self._cplex.variables._conv, ind2), qval, name)
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a):
            return CPX_PROC.getqconstrname(self._env._e, self._cplex._lp, a)
...
class SOSInterface(IndexedInterface):
    ...
    def add(self, type = "1", SOS = SparsePair([0], [0.0]), name = ""):
        ...
        CPX_PROC.addsos(self._env._e, self._cplex._lp, type, [0],
                        map(self._cplex.variables._conv, indices), weights, [name])
...
class MIPStartsInterface(IndexedInterface):
    ...
    def add(self, *args):
        ...
        if len(args) == 1:
            ...
            CPX_PROC.addmipstarts(self._env._e, self._cplex._lp, [0],
                                  map(self._cplex.variables._conv, ind), val, [args[1]], [name])
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a, b = self.get_num() - 1):
            return CPX_PROC.getmipstartname(self._env._e, self._cplex._lp, a, b)
...
class SolnPoolFilterInterface(AdvancedInterface):
    ...
    def add_diversity_filter(self, lb, ub, expression, weights, name):
        ...
        CPX_PROC.addsolnpooldivfilter(self._env._e, self._cplex._lp, lb, ub,
                                      map(self._cplex.variables._conv, ind), weights, val, name)
    ...
    def add_range_filter(self, lb, ub, expression, name):
        ...
        CPX_PROC.addsolnpoolrngfilter(self._env._e, self._cplex._lp, lb, ub,
                                      map(self._cplex.variables._conv, ind), val, name)
    ...
    def get_indices(self, name):
        ...
        """
        if isinstance(name, type("")):
            return CPX_PROC.getsolnpoolfilterindex(self._env._e, self._cplex._lp, name)
        else:
            return map(lambda a:  CPX_PROC.getsolnpoolfilterindex(self._env._e, self._cplex._lp, a), name)
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a):
            return CPX_PROC.getsolnpoolfiltername(self._env._e, self._cplex._lp, a)
...
class SolnPoolInterface(AdvancedInterface):
    ...
    def delete(self, *args):
        ...
        >>> c.solution.pool.get_names()
        ['p1', 'p3', 'p4', 'p5']
        >>> c.solution.pool.delete("p3")
        >>> c.solution.pool.get_names()
        ['p1', 'p4', 'p5']
        >>> c.solution.pool.delete(["p4", 0])
        >>> c.solution.pool.get_names()
        ['p5']
    ...
    def get_indices(self, name):
        ...
        >>> c.populate_solution_pool()
        >>> c.solution.pool.get_indices('p2')
        1
        >>> c.solution.pool.get_indices(["p5", "p3"])
        [4, 2]

        """
        if isinstance(name, type("")):
            return CPX_PROC.getsolnpoolsolnindex(self._env._e, self._cplex._lp, name)
        else:
            return map(lambda a:  CPX_PROC.getsolnpoolsolnindex(self._env._e, self._cplex._lp, a), name)
    ...
    def get_names(self, *args):
        ...
        >>> c.solution.pool.get_names(1)
        'p2'
        >>> c.solution.pool.get_names([1,2])
        ['p2', 'p3']
        >>> c.solution.pool.get_names(1, 4)
        ['p2', 'p3', 'p4', 'p5']

        """
        def getname(a):
            return CPX_PROC.getsolnpoolsolnname(self._env._e, self._cplex._lp, a)
...
class InitialInterface(BaseInterface):
    ...
    def set_start(self, col_status, row_status, col_primal, row_primal,
        ...
        The arguments col_status, col_primal, and col_dual are lists with length
        equal to the number of variables.

        The arguments row_status, row_primal, and row_dual are lists with length
        equal to the number of linear constraints.
