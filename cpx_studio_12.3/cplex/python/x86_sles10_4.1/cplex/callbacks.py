...
from _internal._constants import CPX_CALLBACK_INFO_PRESOLVE_COEFFS
...
class Callback:
    ...
    def _get_col_index(self, name):
        """non-public"""
        status = cb_getcolindex(self._cbstruct, self._env_lp_ptr, name)
        _internal._procedural.check_status(self._cbstruct, status[0], 1)
        return status[1]
        
    def _get_row_index(self, name):
        """non-public"""
        status = cb_getrowindex(self._cbstruct, self._env_lp_ptr, name)
        _internal._procedural.check_status(self._cbstruct, status[0], 1)
        return status[1]
        
    def _get_quad_index(self, name):
        """non-public"""
        status = cb_getqconstrindex(self._cbstruct, self._env_lp_ptr, name)
        _internal._procedural.check_status(self._cbstruct, status[0], 1)
        return status[1]
        
    def _get_sos_index(self, name):
        """non-public"""
        status = cb_getsosindex(self._cbstruct, self._env_lp_ptr, name)
        _internal._procedural.check_status(self._cbstruct, status[0], 1)
        return status[1]
...
class MIPCallback(MIPInfoCallback):
    ...
    class cut_type:
        """Arguments to MIPCallback.get_num_cuts()"""
...
class NodeCallback(MIPCallback):
    ...
    def get_depth(self, which_node):
        ...
        """
        return self._get_node_info(_internal._constants.CPX_CALLBACK_INFO_NODE_DEPTH,
                                   self.__conditionally_convert(which_node))
...
class PresolveCallback(OptimizationCallback):
    ...
    def get_num_modified_coefficients(self):
        ...
        return fast_getcallbackinfo(self._cbstruct, CPX_CALLBACK_INFO_PRESOLVE_COEFFS, exceptions.CplexSolverError)
