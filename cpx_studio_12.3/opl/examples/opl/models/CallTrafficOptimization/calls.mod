...
main {
   ...
   if(cplex.solve()) {
      ...
      var linksdata = new IloOplDataSource("links.dat");
      ...
      while(!(Math.abs(hop.surplus.solutionValue - msol) < EPS)) {
         ...
         if(test == 1) {
            ...
            if(subcplex.solve() && hop.infeas.solutionValue < EPS) { // If the problem is feasible at current Maxhops, test =0;  
               writeln(maxHops, " is feasible; changing to optimization problem");
               test = 0;
               maxHops--;
            } else {
