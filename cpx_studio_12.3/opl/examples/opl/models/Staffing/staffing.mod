...
int Req[Weekdays][Shifts][Skills] = ...;  // # of persons of a skill type required in each shifts
...
dvar int Personnel[Weekdays][Shifts] in 0..card(People);     // # of avail personnel in a shift
...
subject to {
  ...
  // the sum of the number of starting personnel in pervious "nbshifts" shifts.
  ...
  // the total # of person working in a shift <= the # of personnel in the shift.
  ...
  forall(w in Weekdays, s in Shifts,p in People) {
    // If a person start in shift k, she will be available during next nbshifts shifts. 
  ...
  // Everyone has to work no longer than 8 hours (2 shifts in this case)
  ...
  // If a person is on night shift, he cannot be assigned to the morning
  //  shift the next day
  ...
  //Each shift at least has a cooker, a cleaner and a cashier
  forall(w in Weekdays, s in Shifts, j in Skills) 
    ctSkills:
