...
//
// A set of resources (tuple ResourceDat) of different types are available to perform some requests.
// Different types are requests are considered (tuple RequestType). 
...
// Each task is associated a processing time (tuple Task) and a set of resource requirements (tuple Requirement).
...
// is modeled as an interval variable. 
// A request spans its tasks. 
// Allocations are optional.
// Each requirement posts a generalized alternative constraints between 
...
//
// Each resource is a sequence of its non-overlaping allocations.
//
...
// the number of resource of a certain type simultaneously used by the tasks. 
