...
class ILO_EXPORTED IloMultiCriterionExprI : public IloEnvObjectI {
public:
  typedef enum {
    StaticLex = 0,
    DynamicLex = 1
...
public:
  ...
  IloInt getNumCriteria() const { return _exprs.getSize(); }
  IloNumExprI * getCriterion(IloInt i) const {
    IloAssert(i < _exprs.getSize(),
              "IloMultiCriterionExprI: criterion index out of range");
    return _exprs[i].getImpl();
  }
  IloNumExprArray getExprs() const  { return _exprs; }
  IloMultiCriterionExprI::Type getType() const { return _type; }
  void display(ILOSTD(ostream)& out) const;
  const char * getName() const { return _name; }
  void setName(const char * name);
  IloMultiCriterionExprI * duplicate(IloEnvI * env) const;
  IloMultiCriterionExprI * makeClone(IloEnvI * env) const;
};
