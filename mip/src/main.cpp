...
IloNumExpr secondarySumExpr(IloEnv env, IloNumArray coeff,
		IloArray<IloNumVarArray> matrix, int secondIndex, int coeffsize) {
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		for (int k = 0; k < nj; k++) { // only go to jmaxindex?
			j = k + 1;
			while (j <= jmaxindex) {
				if (pj[j] < pj[k] and sj[j] + sj[k] <= capacity
						and Lj[j] >= Lj[k]) {
...
// get out of while loop
		...
		for (int j = 0; j < nj; j++) {
			for (int k = 0; k < nk; k++) {
				model.add(
						IloRange(env, 0, Pk[k] - pj[j] * xjk[j][k],
								IloInfinity));
			}
		}
			...
			for (int j = 0; j < nj; j++) {
				model.add(
						IloRange(env, -IloInfinity,
								Dk[k] - dj[j] * xjk[j][k] + Dmax * xjk[j][k],
								Dmax));
			}
		}
		...
		cplex.setParam(IloCplex::MIPDisplay, 2); // MIP node log display information
    cplex.setParam(IloCplex::TiLim, 8);
...
/*
		...
		// print batch results
		for (int k = 0; k < nk; k++) {
			cout << "Batch " << k << ":\t Pk=" << cplex.getValue(Pk[k])
					<< "\t Dk=" << cplex.getValue(Dk[k]) << "\t Ck="
					<< cplex.getValue(Ck[k]) << endl;
		}
