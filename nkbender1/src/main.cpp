...
#include "Job.h" define FUZZ 0.1

using namespace std;

typedef IloArray<IloBoolVarArray> xjh_matrix; typedef IloArray<IloNumArray> xjh_nummatrix;
...
// quick macro to sum over xjh in both directions (batches and jobs), since IloSum only sums over batches IloNumExpr 
...
int main(int argc, char *argv[]) {
  ...
  int fromHereIgnore = nj;
  for (int j=nj-1; j >= 0; j--) {
    if(Lksingle[j] == IloMax(Lksingle)) {
      fromHereIgnore = j+1;
      //cout << "Ignore after " << fromHereIgnore << endl;
      break;
    }
  }

	/****** The following code used to calculate an upper bound on Lmax. It works well, but it isn't actually used in the model 
right now. *****/
    ...
    // moves that worsen Lmax from the start are prohibited
      // go through all jobs and possible moves, determine if they worsen Lmax
    for(int j=fromHereIgnore-1; j<fromHereIgnore; j++) {
      for(int k=0; k<j; k++) {
        // don't worry about this if it doesn't fit anyway
        if(sj[j] + sj[k] > capacity) continue;
        // don't worry about this if it's a safe move, those are always fine
        if(pj[j] < pj[k]) continue;
        // otherwise, check if the lateness between k and j is increased beyond
        // Lmax_single
        for(int kb=k; kb<j; kb++) {
          // calculate increased L_single
          if(Lksingle[kb] + pj[j] - pj[k] >= IloMax(Lksingle)) {
            mip_cj.add(xjh[j][k] == 0);
            cout << "Detected illegal move." << endl;
            break;
          }
        }
      }
    ...
    float LmaxLB_heuristic = -IloInfinity;
		for (int j = 0; j < nj; j++) {
			...
			if (Cmax_LB_temp - dj[j] > LmaxLB_heuristic) {
				LmaxLB_heuristic = Cmax_LB_temp - dj[j];
				//cout << "Updating LmaxLB to " << LmaxLB_heuristic << ". C="
				//		<< Cmax_LB_temp << ", d=" << dj[j];

			}
		}
    if(LmaxLB_heuristic > 0) {
      LmaxLB = 0;
    } else {
		  LmaxLB = -IloInfinity;
    }
...
xjh[j2][k2])
    ...
    //mip_cj_solver.setParam(IloCplex::LongParam::IntSolLim, 1);
		mip_cj_solver.setParam(IloCplex::MIPDisplay, 2); // MIP node log display information
		mip_cj_solver.setParam(IloCplex::MIPInterval, 1); // Controls the frequency of node logging when the MIP display 
		mip_cj_solver.setParam(IloCplex::Threads, 1);
		mip_cj_solver.setParam(IloCplex::TiLim, 8);
		mip_cj_solver.setParam(IloCplex::TuningTiLim, 100);
    mip_cj_solver.setParam(IloCplex::Probe, 3);
 // mip_cj_solver.setParam(IloCplex::MIPEmphasis, 2); // try for optimality
		mip_cj_solver.solve();
