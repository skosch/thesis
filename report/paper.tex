...
This is where all the CP constraints are.

\subsection{Bin-packing constraint}
...
These constraints are implemented using \texttt{IntervalVariable} objects, which
...
The second constraint ensures that the earliest-due job sets the due date. This
constraint relies on the matrix $x_{jk}$ representing the job-batch assignments,
and the value $\min(d_j x_{jk})$ for a given batch $k$ is always zero (unless said
batch contains all jobs). As a result, this was implemented in a custom
propagator routine.

The third constraints defines $\Lmax$.
...
time. The next batch will then have to be at least as long as the first job we
assign to it, giving us another cardinality constraint. We can continue doing
this until all jobs are assigned to batches. {\color{darkred} This can surely be
...
different processing times as well. If $m$ out of $n_j$ jobs have different
...
Given any partial assignment of jobs, we can reason that
\begin{alist}
...
\subsection{Cumulative constraint}
